package uniquee.enchantments.simple;

import java.util.UUID;

import net.minecraft.enchantment.EnchantmentType;
import net.minecraft.inventory.EquipmentSlotType;
import uniquebase.api.UniqueEnchantment;
import uniquebase.utils.DoubleStat;
import uniquebase.utils.IntStat;
import uniquebase.utils.SlotUUID;

public class Vitae extends UniqueEnchantment
{
	public static final UUID HEALTH_MOD = UUID.fromString("3b538121-d4d7-4aa3-8354-e9849f43526a");
	public static final SlotUUID TRANSCENDED_HEALTH_MOD = new SlotUUID(
			"cacfc3c0-bb1d-402e-9509-e961af0776c5", "1ae23c8c-1b59-4bcc-85c3-e5aa5ce75fe1", "aa0db628-1082-42dc-8f97-bb935ed325c2",
			"72a87567-c59a-42a8-90e7-1ce787d42ad7", "11908392-e901-4578-9913-af800f6c4b8d", "1140ac9d-2b67-41b2-a1bc-1d9b4f3a6fb8");
	public static final IntStat BASE_BOOST = new IntStat(5, "base_boost");
	public static final DoubleStat SCALE_BOOST = new DoubleStat(1D, "scale_boost");
	public static final DoubleStat TRANSCENDED_HEALTH_BOOST = new DoubleStat(1.0, "transcended_health");
	
	public Vitae()
	{
		super(new DefaultData("vitae", Rarity.RARE, 5, false, false, 18, 8, 5), EnchantmentType.ARMOR, EquipmentSlotType.HEAD, EquipmentSlotType.CHEST, EquipmentSlotType.LEGS, EquipmentSlotType.FEET);
		addStats(BASE_BOOST, SCALE_BOOST, TRANSCENDED_HEALTH_BOOST);
	}
}