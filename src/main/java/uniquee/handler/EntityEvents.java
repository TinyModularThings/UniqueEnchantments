...
public class EntityEvents
{
	...
	public void onPlayerTick(PlayerTickEvent event)
	{
		...
		if(event.side == Side.SERVER)
		{
			if(player.getHealth() < player.getMaxHealth())
			{
				...
				if(level > 0 && player.world.getTotalWorldTime() % Math.max((int)(NaturesGrace.DELAY.get() / MathCache.LOG101.get(level)), 1) == 0)
				{
			...
			if(player.world.getTotalWorldTime() % 100 == 0)
			{
				...
				if(level > 0)
				{
					StackUtils.setInt(player.getHeldItemMainhand(), EndestReap.REAP_STORAGE, player.getEntityData().getCompoundTag(EntityPlayer.PERSISTED_NBT_TAG).getInteger(EndestReap.REAP_STORAGE));
				}
			}
			...
			if(player.world.getTotalWorldTime() % 40 == 0)
			{
				...
				if(level > 0)
				{
					List<EntityAgeable> living = player.world.getEntitiesWithinAABB(EntityAgeable.class, new AxisAlignedBB(player.getPosition()).grow(PestilencesOdium.RADIUS.get(MathCache.LOG_ADD_MAX.get(level))));
		...
		if(level > 0)
		{
			...
			if(nbt.getBoolean(Cloudwalker.ENABLED))
			{
				...
				if(!player.isCreative())
				{
					...
					if(player.world.getTotalWorldTime() % Math.max(1, (int)(20 * (Math.sqrt(level) / (leviLevel+1)))) == 0)
					{
	...
	public void onBlockLoot(HarvestDropsEvent event)
	{
		...
		if(level > 0)
		{
			...
			if(stored > 0)
			{
				double extra = ((level*level)/level)+1D;
	...
	public void onArmorDamage(LivingHurtEvent event)
	{
		...
		if(entity instanceof EntityLivingBase)
		{
			...
			if(level > 0)
			{
				...
				if(attr != null)
				{
					event.setAmount(event.getAmount() * (float)Math.log10(10D + (1.6D + Math.log(Math.max(0.25D, attr.getAttributeValue())) / SwiftBlade.BASE_SPEED.get()) * MathCache.LOG.get(level*level)));
				}
			}
			...
			if(level > 0)
			{
				...
				if(attr != null)
				{
					event.setAmount(event.getAmount() * (1F + (float)Math.log10(Math.pow(FocusImpact.BASE_SPEED.get() / attr.getAttributeValue(), 2D)*MathCache.LOG.get(6+level))));
				}
			}
	...
	public void onEntityDamage(LivingDamageEvent event)
	{
		...
		if(entity instanceof EntityLivingBase)
		{
			...
			if(level > 0)
			{
				 event.setAmount(event.getAmount() + (EndestReap.BONUS_DAMAGE_LEVEL.getFloat(level) + EndestReap.REAP_MULTIPLIER.getFloat(level * base.getEntityData().getCompoundTag(EntityPlayer.PERSISTED_NBT_TAG).getInteger(EndestReap.REAP_STORAGE))));
			}
		}
		...
		if(event.getAmount() >= event.getEntityLiving().getHealth())
		{
			...
			if(!source.isMagicDamage() && source != DamageSource.FALL)
			{
				...
				if(level > 0 && stack.isItemStackDamageable())
				{
					...
					stack.damageItem((int)(damage * AresBlessing.BASE_DAMAGE.get() / MathCache.LOG.get(level+1)), event.getEntityLiving());
					event.setCanceled(true);
					return;
				}	
			}
		...
		if(entity instanceof EntityLivingBase)
		{
			...
			if(killed != null && base instanceof EntityPlayer)
			{
				...
				if(amount > 0)
				{
					...
					if(level > 0)
					{
						NBTTagCompound nbt = MiscUtil.getPersistentData(entity);
	...
	public void onArrowHit(ProjectileImpactEvent.Arrow event)
	{
		...
		if(arrow.shootingEntity instanceof EntityPlayer)
		{
			...
			if(slot.getIntValue() > 0)
			{
				...
				int needed = Math.min(MathHelper.floor(MathCache.LOG_ADD.get(level)*EnderMarksmen.EXTRA_DURABILITY.get()), stack.getItemDamage());
				if(needed > 0)
				{
	...
	private Multimap<String, AttributeModifier> createModifiersFromStack(ItemStack stack, EntityLivingBase living, EntityEquipmentSlot slot)
	{
		...
		if(level > 0 && MiscUtil.getSlotsFor(UniqueEnchantments.DEATHS_ODIUM).contains(slot))
		{
			...
			if(value > 0)
			{
				mods.put(SharedMonsterAttributes.MAX_HEALTH.getName(), new AttributeModifier(DeathsOdium.GENERAL_MOD.getId(slot), "Death Odiums Restore", DeathsOdium.BASE_LOSS.get(value), 0));
			}
		}
		return mods;
	}
