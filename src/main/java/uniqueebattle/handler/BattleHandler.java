...
public class BattleHandler
{
	...
	public void onArmorDamage(LivingHurtEvent event)
	{
		...
		if(entity instanceof LivingEntity)
		{
			...
			if(level > 0 && source instanceof PlayerEntity)
			{
				...
				float damage = (float)Math.log(3 + (level * ((1+event.getEntityLiving().getTotalArmorValue()+event.getEntityLiving().getAttribute(Attributes.ARMOR_TOUGHNESS).getValue())*EnchantmentAresFragment.ARMOR_PERCENTAGE.get())));
				while(chance > 0)
				{
	...
	private Multimap<Attribute, AttributeModifier> createModifiersFromStack(ItemStack stack, EquipmentSlotType slot, World world)
	{
		...
		if(level > 0)
		{
			mods.put(Attributes.ATTACK_SPEED, new AttributeModifier(EnchantmentCelestialBlessing.SPEED_MOD, "speed_boost", world.isDaytime() ? 0F : EnchantmentCelestialBlessing.SPEED_BONUS.getAsDouble(level), Operation.MULTIPLY_TOTAL));
		}
		return mods;
	}
}
