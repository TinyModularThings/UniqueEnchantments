	...
	public void onEntityDamage(LivingDamageEvent event)
	{
		if(event.getAmount() >= 1F)
		{
			...
			if(level > 0)
			{
				if(event.getEntityLiving() instanceof EntityPlayer)
				{
					MiscUtil.drainExperience((EntityPlayer)event.getEntityLiving(), MathHelper.ceil(event.getAmount()));
				}
		...
		if(level > 0)
		{
			if(!event.getWorld().isRemote)
			{
				...
				if(nbt.getLong(Resonance.COOLDOWN_TAG) > event.getWorld().getTotalWorldTime())
				{
					...
					if(!player.world.isRemote) player.sendMessage(new TextComponentTranslation("tooltip.uniqueeutil.resonance.cooldown", (nbt.getLong(Resonance.COOLDOWN_TAG) - event.getWorld().getTotalWorldTime()) / 20));
				}
				else
				{
