...
public class BaseHandler
{
	...
	public void onClientTick(ClientTickEvent event)
	{
		...
		if(UniqueEnchantmentsBase.ENCHANTMENT_GUI.test(mc.player) && mc.currentScreen instanceof ContainerScreen && !(mc.currentScreen instanceof EnchantmentGui))
		{
			Slot slot = ((ContainerScreen<?>)mc.currentScreen).getSlotUnderMouse();
			if(slot != null && slot.getHasStack() && EnchantmentHelper.getEnchantments(slot.getStack()).size() > 0)
			{
				tooltipCounter++;
				if(tooltipCounter >= 40) {
					mc.displayGuiScreen(new EnchantmentGui(slot.getStack().copy(), mc.player));
				}
			}
			else
			{
